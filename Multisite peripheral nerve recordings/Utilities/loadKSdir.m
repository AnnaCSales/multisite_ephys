...
flist = [exist(fullfile(ksDir, 'cluster_groups.csv')) , exist(fullfile(ksDir, 'cluster_group.tsv'),'file')];
if any(flist)
    if flist(1)~=0
        [cids, cgs] = readClusterGroupsCSV(fullfile(ksDir, 'cluster_groups.csv'));
    elseif flist(2)~=0
        [cids, cgs] = readClusterGroupsCSV(fullfile(ksDir, 'cluster_group.tsv'));
    end

    if params.excludeNoise
       % noiseClusters = cids(cgs==0);
        noiseClusters = cids(cgs~=2);   %AS edited
...
tempsInd = readNPY(fullfile(ksDir, 'templates_ind.npy'));
tempFeats = readNPY(fullfile(ksDir, 'template_features.npy'));
tempFeatsInd = readNPY(fullfile(ksDir, 'template_feature_ind.npy'));

winv = readNPY(fullfile(ksDir, 'whitening_mat_inv.npy'));
spikeStruct.chanMap=readNPY(fullfile(ksDir, 'channel_map.npy'));
spikeStruct.chanPos=readNPY(fullfile(ksDir, 'channel_positions.npy'));
