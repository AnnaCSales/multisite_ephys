%% Plots spikes for a list of selected clusters around TTL - for comparing clustered data 
 % with raw heatmap data. Plots each cluster in a different colour, as in
 % Supp 4(c)

%% specify TTLs to plot
TTLchan1=2;  %this is the channel to take TTLs from
% fsts=spikeStruct.TTLs.allTTLs;
...
win=[0.25, 0.2] ; %specify a window, time before and after event to consider for pinches
binwin=0.002;

% clusters to plot:
sel_clu=[2,3];

% colours to use (make sure there are enough colours to match the cluster list):
cols=[rgb('Red'); rgb('Blue');rgb('Green'); rgb('Orange')]
...
for g=1:length(sel_clu)
    this_clu=sel_clu(g);
    this_clu_ids=spikeStruct.cids(this_clu);
    spike_inds=find(ismember(spikeStruct.clu, this_clu_ids));
    ...
    for iTTL=1:nTTL %footshocks - clean this up later, this is a long winded way of doing this
        ...
        if ts_window
            plot(1000*ts_plot, y_marks_', 'color',this_col, 'LineWidth', 1);
        end
        hold on
        yticks(0:40:nTTL);
